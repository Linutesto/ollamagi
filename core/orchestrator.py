...
def _fix_python(code: str, error_output: str, description: str, flow_id: str | None) -> str:
    ...
    prompt = (
        ...
        "The BUILD SCRIPT itself must parse as Python. NEVER use triple-quoted strings (''' or \"\"\"); "
        "nested docstrings terminate the outer string. NEVER write a string literal that spans more "
        "than one physical line without a backslash continuation — an unterminated string literal "
        "crashes the entire build script before a single line executes. "
        "Write file content using a list of ordinary single-quoted strings joined with '\\n', "
        "json.dumps(), repr(), or multiple Path.write_text() calls. Do not use ''' or \\\"\\\"\\\" anywhere.\n"
...
def _execute_subtask(subtask: Subtask, flow: Flow, task: Task,
    ...
    if use_python:
        code_prompt = (
            ...
            "- NEVER use triple-quoted strings (''' or \"\"\") anywhere in the build script; "
            "nested docstrings terminate the outer string. Write multi-line content as a list of "
            "ordinary single-quoted strings joined with '\\n', or use json.dumps() / repr() for "
            "safe encoding, or write content with multiple Path.write_text() calls\n"
            "- NEVER write a string literal that spans more than one physical line without a "
            "backslash continuation. An unterminated string literal crashes the entire build "
            "script before a single line executes. When in doubt, encode with repr() or json.dumps()\n"
